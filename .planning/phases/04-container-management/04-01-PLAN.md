...
must_haves:
  truths:
    ...
    - "Concurrent lifecycle actions on the same container are prevented via Redis lock"
    - "DatabaseService provides query methods for listing containers with relations and getting aggregate counts"
    - "DatabaseService provides createContainerEvent and updateContainerLifecycle methods"
  ...
  artifacts:
    ...
    - path: "apps/dashboard/src/lib/db.ts"
      provides: "listContainersWithRelations, getContainerCounts, getContainerWithDetails, deleteContainerById, updateContainerServices, createContainerEvent, updateContainerLifecycle methods"
...
6. **`createContainerEvent(data: { containerId: string; type: string; message: string; metadata?: any })`** — Creates a ContainerEvent record. Used by lifecycle actions and monitoring. If Phase 3 already added this, skip — but include it here for self-sufficiency.
...
static async createContainerEvent(data: {
  ...
  return this.prisma.containerEvent.create({
    data: {
...
7. **`updateContainerLifecycle(containerId: string, lifecycle: "creating" | "ready" | "error")`** — Updates a container's lifecycle field. Used by lifecycle actions when errors occur.
...
Note: If Phase 3 already added `getContainerById` with includes, `getContainerWithDetails` is a more comprehensive version. Keep both — `getContainerById` is used by the worker (simpler), `getContainerWithDetails` is for the detail page (full). Similarly, if Phase 3 already added `createContainerEvent` or `updateContainerLifecycle`, keep the existing implementations — these definitions ensure self-sufficiency if Phase 3 didn't create them.
  ...
  <verify>
    ...
    - DatabaseService has methods: listContainersWithRelations, getContainerCounts, getContainerWithDetails, deleteContainerById, updateContainerServices, createContainerEvent, updateContainerLifecycle
  </verify>
  <done>
    - createProxmoxClientFromSession helper encapsulates session → client pattern
    - DatabaseService has all query methods needed by dashboard (list+counts), detail page (full details), delete action, service monitoring (bulk update), and lifecycle actions (createContainerEvent, updateContainerLifecycle)
...
**Concurrent Prevention — Redis Lock:**

Import Redis from `@/lib/redis` (already available from Phase 2 infrastructure). Add a lock helper used by ALL lifecycle actions:
...
async function acquireContainerLock(
  ...
  // SET NX = set-if-not-exists, EX 120 = expire after 120 seconds (safety timeout)
  const result = await redis.set(
    `container:${containerId}:action`,
    action,
    "EX",
    120,
    "NX",
...
async function releaseContainerLock(containerId: string): Promise<void> {
  await redis.del(`container:${containerId}:action`);
}
```

Every lifecycle action MUST:

1. Call `acquireContainerLock(containerId, "start"|"stop"|etc.)` at the top
2. If lock not acquired, immediately return `{ success: false, error: "Another operation is already in progress on this container" }`
3. Wrap the entire action body in a `try/finally` that calls `releaseContainerLock(containerId)` in the `finally` block

**Create these server actions:**

1. **`startContainerAction(containerId: string): Promise<ActionResult>`**
   - Acquire lock: `acquireContainerLock(containerId, "start")` — return error if lock not acquired
   - `try {` — all remaining steps in try block
   ...
   - `} finally { await releaseContainerLock(containerId); }`

2. **`stopContainerAction(containerId: string): Promise<ActionResult>`**
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
   - Get client, get container, check status (error if already stopped)
...
3. **`shutdownContainerAction(containerId: string): Promise<ActionResult>`**
   - Same lock pattern: acquire lock → try/finally with releaseContainerLock
   - Get client, get container, check status (error if not running)
...
**Revalidation:** After each successful action, call `revalidatePath("/")` and `revalidatePath("/containers/[id]")` (use the actual format `revalidatePath("/containers/" + containerId)`) to trigger Next.js cache refresh. Import from `next/cache`.
  ...
  <verify>
    ...
    - Each action acquires Redis lock before Proxmox calls and releases in finally block
    - Each action returns `{ success: false, error: "Another operation is already in progress..." }` when lock not acquired
  ...
  <done>
    ...
    - Concurrent actions on the same container are prevented via Redis lock (SET NX EX 120)
    - Lock acquired at start, released in finally block (even on error)
...
3. DatabaseService has: listContainersWithRelations, getContainerCounts, getContainerWithDetails, deleteContainerById, updateContainerServices, createContainerEvent, updateContainerLifecycle
4. 5 lifecycle server actions exist with proper state validation and event logging
5. Each action uses Redis lock to prevent concurrent operations on same container
6. Shutdown has graceful→force fallback
7. Delete stops running containers before deletion
8. All actions call revalidatePath for cache refresh
</verification>

<success_criteria>

- Helper provides reusable ProxmoxClient creation from session (no more copy-paste pattern)
- DatabaseService extended with all query methods Wave 2 UI needs, plus createContainerEvent and updateContainerLifecycle
- 5 lifecycle actions work: start/stop/shutdown/restart/delete with proper error handling
- Concurrent actions on same container prevented via Redis lock (SET NX EX 120, released in finally)
