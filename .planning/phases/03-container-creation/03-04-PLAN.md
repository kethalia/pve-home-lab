...
must_haves:
  truths:
    ...
    - "Progress page shows completion state with discovered services, credentials, and link to container"
...
- **Completion state** (status === "complete"): Show success Card with green checkmark, "Container Ready!" message. Below the success message, show:
  - **Discovered Services card:** Fetch container services via a server action or API call (`DatabaseService.getContainerServices(containerId)`) and display as a list of service cards, each showing: service name, type badge (systemd/docker/process), port number, and a clickable web URL link if available. If no services discovered, show "No services detected" with muted text.
  - **Credentials card:** For services with credentials, show a collapsible card with the service name and a "Show Credentials" button that reveals the credentials (fetch and decrypt on demand). Use a copy button for each credential value.
  - Action buttons: "View Container" (link to `/containers/${id}`), "Create Another" (link to `/containers/new`)
...
8. Completion state displays discovered services with ports and web URLs
9. Completion state shows credential cards with show/copy functionality
</verification>
...
- Completion state shows success with discovered services, credential cards, and navigation links
