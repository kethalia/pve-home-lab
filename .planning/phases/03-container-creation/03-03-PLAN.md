...
must_haves:
  truths:
    ...
    - "User can configure hostname, VMID, password, resources, storage, network, and features"
...
<task type="auto">
  <name>Task 2: Container wizard client component with 5 step forms</name>
  <files>
    apps/dashboard/src/app/(dashboard)/containers/new/container-wizard.tsx
    apps/dashboard/src/app/(dashboard)/containers/new/wizard-stepper.tsx
    apps/dashboard/src/app/(dashboard)/containers/new/steps/template-step.tsx
...
Each step is a separate file in `steps/` directory. All are `"use client"` components.

**Step 1: template-step.tsx**
...
**Step 2: configure-step.tsx**

- Form with sections:
  - **Identity:** hostname (Input), VMID (Input, pre-filled with nextVmid)
  - **Access:** root password (Input type="password"), confirm password, SSH public key (Textarea, optional)
...
**Styling:** Use shadcn/ui components (Card, Button, Input, Label, Select, Switch, Checkbox, Badge, Textarea, Separator). Use Tailwind for layout — responsive grid for template cards (2-3 cols on lg, 1 on mobile). Keep form sections visually separated with Separator or Card boundaries.

**Do NOT install any additional packages** — use only existing shadcn/ui components and Tailwind.
</action>
<verify> - `npx tsc --noEmit` passes - All 7 files exist in the containers/new/ directory - `grep "use client" apps/dashboard/src/app/\(dashboard\)/containers/new/container-wizard.tsx` confirms client component - Each step file exports a named component - Navigate to /containers/new in the browser shows the wizard (requires running dev server)
</verify>
<done> - 5-step wizard renders at /containers/new with horizontal step indicator - Step 1: Template selection grid with "start from scratch" option - Step 2: Full configuration form with Zod validation and inline errors - Step 3: Package bucket toggles with expandable package lists - Step 4: Script enable/disable with up/down reorder buttons - Step 5: Review summary with Deploy button - Template defaults pre-populate Steps 2-4 when a template is selected - Deploy button calls server action and navigates to progress page - All navigation (back/next) preserves state
</done>
</task>
...
- Review step shows complete summary of all configuration
