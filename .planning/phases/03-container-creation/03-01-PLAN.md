...
must_haves:
  truths:
    ...
    - "DatabaseService has Container, ContainerEvent, and ContainerService CRUD methods"
  artifacts:
    ...
    - path: "apps/dashboard/src/lib/db.ts"
      provides: "DatabaseService with Container, ContainerEvent, ContainerService methods"
  ...
  key_links:
    ...
    - from: "apps/dashboard/src/lib/db.ts"
      ...
      via: "DatabaseService methods operate on Container, ContainerEvent, ContainerService models"
      pattern: "prisma\\.container\\.|prisma\\.containerEvent\\.|prisma\\.containerService\\."
---
...
<task type="auto">
  <name>Task 3: Add Container, ContainerEvent, and ContainerService DatabaseService methods</name>
  <files>
    apps/dashboard/src/lib/db.ts
  </files>
  <action>
Extend `src/lib/db.ts` with Container, ContainerEvent, and ContainerService methods. This is done in Plan 01 (Wave 1) so both Plan 02 (worker) and Plan 03 (wizard) can use these methods in parallel during Wave 2.
...
**2. Add Container + ContainerEvent + ContainerService methods to DatabaseService:**
...
static async getContainerServices(containerId: string): Promise<ContainerService[]> {
...
Also import the enums as values (not just types) since they're needed at runtime:
...
Re-export the enums from db.ts so the worker and other consumers can import them:
...
```
  ...
  <verify>
    ...
    - DatabaseService has createContainer, getContainerById, updateContainerLifecycle, createContainerEvent, getContainerEvents, createContainerService, getContainerServices methods
    - ContainerLifecycle, EventType, ServiceType, ServiceStatus enums are re-exported
  </verify>
  <done>
    - prisma instance exported for direct use in complex transactions
    - DatabaseService has Container CRUD, ContainerEvent, and ContainerService methods
    - ContainerLifecycle, EventType, ServiceType, ServiceStatus enums re-exported for worker and wizard use
    - Both Plan 02 (worker) and Plan 03 (wizard) can depend on these methods from Wave 1
  </done>
</task>
...
6. DatabaseService has Container, ContainerEvent, and ContainerService CRUD methods
7. prisma instance exported for direct access
8. ContainerLifecycle, EventType, ServiceType, ServiceStatus enums re-exported
</verification>
...
- DatabaseService has Container, ContainerEvent, and ContainerService CRUD methods ready for Wave 2 plans
- prisma instance is exported for direct use in complex worker transactions
- Enums (ContainerLifecycle, EventType, ServiceType, ServiceStatus) are re-exported for consumer use
  </success_criteria>
