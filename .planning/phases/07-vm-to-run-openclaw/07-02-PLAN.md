...
must_haves:
  truths:
    - "Users can create VM templates through the existing template form"
    - "Template form shows VM-specific configuration options when VM type selected"
    - "OpenClaw VM template with cloud-init automation is ready for deployment"
    - "DatabaseService supports VM template creation with proper validation"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    3. Validate VM-specific constraints (memory ≥ 1GB, etc.)
