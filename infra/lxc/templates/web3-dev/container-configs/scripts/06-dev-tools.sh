...
install_pnpm() {
    ...
    if sudo -u "$CONTAINER_USER" bash -c "command -v pnpm" >/dev/null 2>&1; then
        PNPM_VERSION=$(sudo -u "$CONTAINER_USER" bash -c "pnpm --version" 2>/dev/null || echo "unknown")
        log_info "pnpm is already installed: v${PNPM_VERSION}"
        return 0
    fi
    
    # Check if npm is available
    if ! sudo -u "$CONTAINER_USER" bash -c "command -v npm" >/dev/null 2>&1; then
    ...
    if sudo -u "$CONTAINER_USER" bash -c "
        ...
        command -v pnpm
    " >/dev/null 2>&1; then
        PNPM_VERSION=$(sudo -u "$CONTAINER_USER" bash -c "
            export NVM_DIR='/home/${CONTAINER_USER}/.nvm'
            if [[ -s \"\$NVM_DIR/nvm.sh\" ]]; then
                source \"\$NVM_DIR/nvm.sh\"
            fi
